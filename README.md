...
- **Redact** — the editor reads the screenshot the moment it opens and counts the emails, phone numbers, card numbers (Luhn-checked) and API tokens it can see. Press **Redact** and each one gets its own blur box — sized to the string, not the whole line. Nothing is touched until you press it, `⌘Z` takes the whole batch back out, and a false positive goes away with Select + `⌫`. Vision OCR on-device; the image never leaves the Mac.
