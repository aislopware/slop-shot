...
- **Annotation editor** — shapes, text, arrows, numbered step counters, and a **blur** tool for hiding anything you don't want in the shot (drag a box; the stroke-width control doubles as blur strength); zoom via buttons, `⌘ +/-/0`, and trackpad pinch. Select a layer and press `⌫` to drop it.
- **Redact** — the editor reads the screenshot the moment it opens and counts what it can see: emails, phone numbers, card numbers (Luhn-checked) with their CVV, expiry date and cardholder name, API tokens, labelled passwords/OTPs, and ID/SSN/passport numbers. Anything that needs a label to be meaningful (a bare `231` is not a CVV) is only matched next to one, and names are only touched on an image that already has card data on it. Press **Redact** and each one gets its own blur box — sized to the string, not the whole line. Nothing is touched until you press it, `⌘Z` takes the whole batch back out, and a false positive goes away with Select + `⌫`. Vision OCR on-device; the image never leaves the Mac.
