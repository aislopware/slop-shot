...
[ -n "$MODE" ] && [ -n "$VALUE" ] || {
  ...
  echo "      $0 --range <eid đầu> <eid cuối> [tên bộ]"
  exit 1
}
...
for s in json.load(sys.stdin).get('value',[]):
    ...
    [ -n "$EIDS" ] || {
      echo "cid $VALUE không trả về sticker nào."
      exit 1
    }
    ;;
  --range)
    HI="${3:-}"
    [ -n "$HI" ] || {
      echo "--range cần cả eid đầu và eid cuối."
      exit 1
    }
    ...
    ;;
  *)
    echo "Mode phải là --cid hoặc --range"
    exit 1
    ;;
esac
...
export -f fetch_eid
export API SPRITE
# Nháy đơn là cố ý: "$1" phải nở ra ở bash CON (nó nhận "$DEST" qua đối số), không phải ở đây.
...
echo "✅ $(find "$DEST" -maxdepth 1 -type f | wc -l | tr -d ' ') sticker → $DEST  (động: $anim)"
